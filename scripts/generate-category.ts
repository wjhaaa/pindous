...
5. 内存与性能意识 — 垃圾回收、定时器清理、事件监听清理、闭包引用释放、常见内存泄漏`,
  ...
  "browser-web-api": {
    fileName: "browser-web-api.ts",
    varName: "browserWebApiQuestions",
    idPrefix: "web",
...
3. 常用 Web API — File API、Clipboard API、Web Worker、Service Worker基本概念、History API、Location`,
  ...
  "html-css": {
    fileName: "html-css.ts",
    varName: "htmlCssQuestions",
    idPrefix: "css",
...
2. Hooks 基础 — useState、useEffect、useRef、useMemo、useCallback、useReducer、useContext、自定义 Hook、Hooks调用规则、副作用 Hook
...
3. 路由、React 18 与组件设计 — BrowserRouter/HashRouter、动态路由、权限路由、自动批处理、useTransition、useDeferredValue、Suspense、组件拆分、hooks抽象、通用组件API`,
      ...
      {
        count: 10,
        subCategories: `1. React 测试 — React Testing Library、组件交互测试、Mock 请求、快照测试适用场景
2. React 原理 — JSX 编译、架构演进、双缓冲机制、渲染两阶段、时间切片、优先级模型、并发模式、节点复用、渲染优化、与 Vue 对比`,
      },
    ],
  },

  "taro-mini-program": {
    fileName: "taro-mini-program.ts",
    varName: "taroMiniProgramQuestions",
    idPrefix: "taro",
...
3. Taro 工程与项目实战 — 环境变量、页面注册、构建配置、多端兼容、调试工具、收藏功能、学习进度统计、每日一题、列表筛选、详情页跳转、本地mock`,
  ...
  engineering: {
    fileName: "engineering.ts",
    varName: "engineeringQuestions",
    idPrefix: "eng",
...
function buildPrompt(
  ...
  const existingSummary =
    existing.length > 0
      ? `\n## 已有题目（共 ${existing.length} 道，切勿与以下题目重复！）\n\n` +
        ...
        `\n\n新题目 ID 从 ${category.idPrefix}-${String(startId).padStart(
          3,
          "0",
        )} 开始。\n`
      : "";
  ...
  return `你是一位资深前端面试官兼前端学习教练，熟悉国内互联网公司真实面试，也熟悉前端开发者半年没写项目后重新恢复手感的学习路径。请为「${
    category.displayName
  }」分类生成 ${batch.count} 道前端面试题。
...
10. **ID 格式**：${category.idPrefix}-${String(startId).padStart(3, "0")} 到 ${
    category.idPrefix
  }-${String(endId).padStart(3, "0")}
...
async function callDeepSeek(prompt: string): Promise<NewQuestion[]> {
      ...
      messages: [
        {
          role: "system",
          content:
            "你必须严格按 JSON Schema 输出，只输出 JSON 对象，不得包含任何其他文字。",
        },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `DeepSeek API 错误 ${response.status}: ${errBody.slice(0, 500)}`,
    );
  }
  ...
  if (!Array.isArray(questions)) {
    throw new Error(
      `DeepSeek 返回 JSON 中缺少 questions 数组。keys: ${Object.keys(
        parsed || {},
      ).join(", ")}`,
    );
  }
...
function writeCategoryFile(
  ...
  if (append && fs.existsSync(filePath)) {
    ...
    if (lastClose === -1)
      throw new Error(`无法在 ${category.fileName} 中找到 ];`);
...
function writeIndexFile(): void {
  ...
  for (const [, cat] of Object.entries(CATEGORIES)) {
    if (fs.existsSync(path.join(QUESTIONS_DIR, cat.fileName))) {
      imports.push(
        `import { ${cat.varName} } from "./${cat.fileName.replace(
          ".ts",
          "",
        )}";`,
      );
      exports.push(
        `export { ${cat.varName} } from "./${cat.fileName.replace(
          ".ts",
          "",
        )}";`,
      );
...
async function main() {
  ...
  if (!categoryArg || !CATEGORIES[categoryArg]) {
    console.error(
      "用法: npx tsx scripts/generate-category.ts <category> [--all] [--dry-run]",
    );
    console.error("可用分类: " + Object.keys(CATEGORIES).join(", "));
    console.error("--all: 生成该分类所有批次");
    console.error("--dry-run: 展示题目详情（仍会写入文件）");
    process.exit(1);
  }
  ...
  console.log(
    `   共 ${category.batches.length} 批，目标 ${category.batches.reduce(
      (s, b) => s + b.count,
      0,
    )} 题`,
  );
  console.log(`   ID 前缀：${category.idPrefix}-`);
  if (dryRun) console.log("   Dry run：展示题目内容（仍会写入文件）");
  console.log("");
  ...
  console.log(
    `   执行模式：${
      runAll ? `全部 ${batches.length} 批` : "仅第 1 批（加 --all 执行全部）"
    }`,
  );
  console.log("");

  // 读已有文件（统一的 ID 基数和去重基础）
  const fileExisting = readExistingQuestions(category);
  const fileMaxId = fileExisting.reduce((max, q) => {
    const num = parseInt(q.id.split("-").pop() || "0");
    return Math.max(max, isNaN(num) ? 0 : num);
  }, 0);

  // 预计算每批的起始 ID
  const batchPlans = batches.map((batch, idx) => {
    const offset = batches.slice(0, idx).reduce((s, b) => s + b.count, 0);
    return {
      batch,
      batchIdx: idx,
      startId: fileMaxId + offset + 1,
    };
  });

  let batchFailed = false;

  if (runAll && batches.length > 1) {
    // ── 并行模式（--all 多批次）──────────────────
    console.log(`🚀 并行生成 ${batches.length} 批，共 ${batches.reduce((s, b) => s + b.count, 0)} 题…`);
    console.log("");

    // 并行发起所有 API 调用
    const batchPromises = batchPlans.map(async ({ batch, batchIdx, startId }) => {
      const label = `第 ${batchIdx + 1}/${batches.length} 批（${batch.count} 题，ID ${category.idPrefix}-${String(startId).padStart(3, "0")} 起）`;

      const prompt = buildPrompt(category, batch, fileExisting, startId);
      ...
      if (lastErr) return { batchIdx, startId, label, rawQuestions: [], error: lastErr.message };

      return { batchIdx, startId, label, rawQuestions, error: null };
    });
    ...
    for (const res of sortedResults) {
      ...
      if (res.error) {
        console.log(`   ❌ API 调用失败：${res.error.slice(0, 100)}`);
        console.log("");
        batchFailed = true;
        continue;
      }

      console.log(`   返回了 ${res.rawQuestions.length} 道题`);

      const existingIds = new Set([
        ...fileExisting.map((q) => q.id),
        ...allQuestions.map((q) => q.id),
      ]);
      const { valid, errors } = validateQuestions(res.rawQuestions, category, existingIds, res.startId);
      ...
      console.log(
        `   难度: 简单 ${diffCounts[1]} / 中等 ${diffCounts[2]} / 困难 ${diffCounts[3]}`,
      );
      console.log("");
    ...
    // ── 串行模式（单批）──────────────────
    const { batch, startId } = batchPlans[0];

    const existingIds = new Set(fileExisting.map((q) => q.id));

    console.log(
      `📦 第 1/1 批（${batch.count} 题，ID 从 ${
        category.idPrefix
      }-${String(startId).padStart(3, "0")} 开始）`,
    );
    console.log("");
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        lastErr = e;
        if (attempt < 2)
          console.log(`   ⚠️ 调用失败：${e.message.slice(0, 100)}`);
      }
    }
    ...
    if (valid.length > 0) {
      ...
      console.log(`📝 追加写入 ${category.fileName}…`);
      writeCategoryFile(category, valid, true);
  ...
  if (runAll) {
    if (batchFailed || allQuestions.length === 0) {
      console.log(
        "❌ 部分批次失败或无有效题目，取消写入。请检查上述错误后重试。",
      );
      process.exit(1);
    }
    console.log(
      `📝 追加写入 ${category.fileName}（共 ${allQuestions.length} 题）…`,
    );
    writeCategoryFile(category, allQuestions, true);
  }
