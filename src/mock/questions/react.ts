...
export const reactQuestions: Question[] = [
  ...
  {
    id: "react-016",
    title: "React 中 StrictMode 的作用是什么？",
    type: "single",
    options: ["用于提高应用性能","用于在开发模式下检测潜在问题并发出警告","用于强制组件重新渲染","用于实现严格类型检查"],
    answer: "用于在开发模式下检测潜在问题并发出警告",
    explanation: "StrictMode 是 React 提供的一个工具组件，仅在开发模式下生效。它会帮助检测：1. 不安全的生命周期方法；2. 过时的字符串 ref API；3. 意外的副作用（通过故意双重调用渲染和 useEffect 清理函数）；4. 废弃的 findDOMNode 使用。它不会影响生产环境的渲染，目的是帮助开发者写出更健壮的代码。",
    difficulty: 2,
    tags: ["React","开发工具"],
    category: "React",
  },
  {
    id: "react-017",
    title: "React 19 中新的 `use()` Hook 的作用和使用方式？",
    type: "qa",
    answer: "`use()` 是一个新的 Hook，用于在渲染期间直接读取 Promise 或 Context 的值，无需配合 `useEffect` 或 `useState`。它能够暂停组件的渲染直到 Promise 解析完成，与 Suspense 集成，简化了异步数据获取。语法：`const data = use(promise)` 或 `const contextValue = use(SomeContext)`。注意 `use` 不能在条件或循环中调用，与普通 Hook 规则不同，但它可条件性地调用。",
    explanation: "React 19 的 `use` 可将异步操作无缝集成到组件渲染流中，依靠 Suspense 边界提供加载状态。它简化了数据获取逻辑，替代了以往通过 `useEffect` + `useState` 的模式。",
    difficulty: 3,
    tags: ["React 19","use","异步","Suspense"],
    category: "React",
  },
  {
    id: "react-018",
    title: "React 中 useTransition 和 useDeferredValue 有什么区别？分别的使用场景？",
    type: "qa",
    answer: "useTransition 和 useDeferredValue 都是 React 18 并发特性中用于标记低优先级更新的 Hook。useTransition 返回一个布尔值 isPending 和一个 startTransition 函数，调用 startTransition 包裹状态更新可以让该更新变为可中断的低优先级更新，常用于路由导航或列表筛选等交互，让 UI 保持响应。useDeferredValue 接收一个值并返回该值的延迟版本，当原始值变化时，React 会在空闲时更新延迟值，通常用于展示大量数据时，将输入查询延迟传递给耗时的子组件渲染，避免阻塞用户输入。区别：useTransition 关注状态更新的优先级，而 useDeferredValue 关注值的延迟；前者直接控制更新过程，后者仅返回延迟值供子组件使用。",
    explanation: "两者都是并发模式下的优化手段，但用法不同：useTransition 用于包裹 setState，降低该更新的优先级；useDeferredValue 用于产生一个可延迟同步的值，常用于将高优先级变更（如输入框内容）与昂贵渲染解耦。",
    difficulty: 3,
