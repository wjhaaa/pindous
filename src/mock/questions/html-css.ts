...
export const htmlCssQuestions: Question[] = [
  ...
  {
    id: "html-004",
    title: "HTML 中 link 标签的 preload、prefetch、preconnect 等资源提示有什么区别？",
    type: "qa",
    answer: "preload 用于声明当前页面高优先级资源，告诉浏览器尽快下载并与当前页面并行加载，不会阻塞页面渲染，通常用于字体、关键 CSS 等；prefetch 用于预获取未来可能访问的页面资源，优先级较低，浏览器空闲时加载；preconnect 用于提前与第三方域名建立连接（DNS、TCP、TLS），减少后续请求的延迟。它们触发时机和优先级不同，综合使用可以优化页面加载性能。",
    explanation: "preload 是强制浏览器加载资源，适合当前页面必须尽快使用的资源；prefetch 是暗示浏览器未来可能导航到的资源，不影响当前页面；preconnect 只建立连接，不下载资源，可以与 dns-prefetch 配合；此外还有 prerender（已废弃）。合理使用这些资源提示可以显著提升首屏速度和页间切换体验。",
    difficulty: 2,
    tags: ["HTML","性能优化","资源提示"],
    category: "HTML/CSS",
  },
  {
    id: "css-015",
    title: "CSS Contain 属性有什么作用？取值有哪些？",
    type: "qa",
    answer: "contain 属性用于隔离元素及其子元素的样式、布局和渲染，允许浏览器单独处理该元素，从而提升页面性能。取值包括：layout（元素内部布局不影响外部）、paint（子元素不会超出容器）、size（元素尺寸不依赖子元素）、style（计数器等样式属性不会影响外部）、content（包含 layout、paint、size 等功能）、strict（包含 layout、paint、size、style）。",
    explanation: "contain 属性的核心是让浏览器知道某个元素的内部变化是独立的，从而避免不必要的全局重排、重绘。例如，在渲染长列表时，设置 contain: content 可以大幅提升滚动性能，因为浏览器可以只重新绘制可见区域，而无需考虑影响外部布局。它是 CSS contain 规范的一部分，现代浏览器均基本支持。",
    difficulty: 2,
    tags: ["CSS","性能优化","contain"],
    category: "HTML/CSS",
  },
];
